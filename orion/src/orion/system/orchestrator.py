...
DEFAULT_SYSTEM = ("You are ORION, a careful assistant. Reason internally, then answer clearly and concisely. "
                  "For calculations give the final answer on the last line as '#### <answer>'. "
...
class Orchestrator:
    ...
    def _context(self, user_text: str, session: str, route: Route, evidence: str | None) -> list[dict[str, str]]:
        ...
        if self.memory is not None and (mem := self.memory.memory_prompt(user_text, session)):
            system += "\n\n" + mem
    ...
    def _tool_loop(self, backend, messages: list[dict[str, str]]) -> tuple[list[dict[str, str]], list[dict[str, Any]]]:
    ...
    def answer(self, user_text: str, session: str = "default", attachments: list[Any] | None = None, tests: str | list[str] | None = None) -> OrchestratorResponse:
        ...
        if route.needs_tools and self.tools is not None:
            messages, tool_trace = self._tool_loop(backend, messages)
        ...
        result = engine.run(messages, intent=route.intent, n_candidates=route.candidates,
                            checker=self._math_checker(tool_trace) if route.intent == "math" else None, tests=tests)
        text = result.text
